...
function getFullPath(fileName)
    ...
    fullDBPath = cc.FileUtils:getInstance():fullPathForFilename(fileName)
    print(fullDBPath)
    return fullDBPath
end
-- 解析一行
local function newparseline(line,head)
    local ret = {};
...
local function split(str, reps)  
    local arrs = {};  
    string.gsub(str, '[^' .. reps ..']+', function(w) table.insert(arrs, w) end );  
    return arrs;  
end  
...
function LoadCsv(fileName)
    ...
    local ret = {}
    local file = cc.FileUtils:getInstance():getStringFromFile(fullpath)
    local lineStr = split(file, '\n\r');
    local head = {}
    for k,v in pairs(lineStr) do
        if k == 1 then
            head = parseline(v)            
        else
            local temp = newparseline(v,head)  
            local sKey = tostring(temp[iKey])
            assert(sKey,"csv skey = nil")
            ret[sKey] = temp
        end
    end 
    return ret
