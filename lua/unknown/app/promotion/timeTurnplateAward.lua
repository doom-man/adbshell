timeTurnplateAward = class("timeTurnplateAward", function(...)
    local arg = { ...}
    if table.getn(arg) == 1 then
        return cc.CSLoader:createNode(arg[1])
    elseif table.getn(arg) == 2 then
        -- 这里当当前layer的父节点不在场景中的时候才用clone（）并且该对象必须是UIWidget 子类 一般是从globalItems里面取的时候用
        return arg[1]:getChildByName(arg[2]):clone()
    end
end )
timeTurnplateAward.__index = timeTurnplateAward

function timeTurnplateAward:create(...)
    local layer = timeTurnplateAward.new(...)
    if layer then
        if layer:init() then
            layer:registerScriptHandler( function(tag)
                if "enter" == tag then
                    layer:onEnter()
                elseif "exit" == tag then
                    layer:onExit()
                end
            end )
            return layer
        end
    end
    return nil
end

function timeTurnplateAward:ctor()

end

function timeTurnplateAward:init()
    
    self.ItemPanel = me.assignWidget(self, "ItemPanel")
    self.listView = me.assignWidget(self, "ListView")
    self.listView:setScrollBarPositionFromCornerForVertical(cc.p(2, 2))
    self.closeBtn = me.registGuiClickEventByName(self, "close", function(node)
        me.DelayRun(function (args)
           self:close()
        end)
    end)
    self.awardBtn = me.assignWidget(self, "awardBtn")
    me.registGuiClickEvent(self.awardBtn, function()       
            self:close()        
    end)
    me.assignWidget(self,"patrolBtnTxt") :setString("确定")
    me.assignWidget(self,"Text_Title"):setString("夺宝奖励")
    return true
end

function timeTurnplateAward:setData(data)
    self.listView:removeAllItems()
    self.data = data

    local itemPanel = nil
    local w=0
    for key, var in ipairs(self.data) do
        if (key-1)%5==0 then
            w=0
            itemPanel = self.ItemPanel:clone():setVisible(true)
            self.listView:pushBackCustomItem(itemPanel)
        end
        item = BackpackCell:create("backpack/backpackcell.csb")
        var.defid=var.item[1]
        var.count=var.item[2]
        item:setScale(0.7)
        item:setUI(var)  
        item:setAnchorPoint(0, 0)   
        item:setPosition(w*138+20, 0)  
        local btnBg = me.assignWidget(item,"Button_bg")
        me.registGuiClickEvent(btnBg,function ()
            showPromotion(var.item[1],var.item[2])
        end)  
        btnBg:setSwallowTouches(false)
        if var.item[2]==1 then
            me.assignWidget(item, "num_bg"):setVisible(false)
        end
        itemPanel:addChild(item)
        w=w+1
    end
    
end

function timeTurnplateAward:onEnter()    
    me.doLayout(self,me.winSize)
end

function timeTurnplateAward:onExit()
    print("timeTurnplateAward:onExit()")
end
function timeTurnplateAward:close()
    self:removeFromParentAndCleanup(true)
    
end


